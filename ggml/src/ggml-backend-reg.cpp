...
using dl_handle_ptr = std::unique_ptr<dl_handle, win32_handle_deleter>;
#else
...
#ifdef _WIN32
static dl_handle_ptr dl_load_library(const std::wstring & path, bool silent) {
    ...
    if (!handle) {
        if (!silent) {
            GGML_LOG_ERROR("%s: failed to load %s: %lu\n", __func__, "path.c_str()", GetLastError());  // FIXME
        }
        return nullptr;
    }

    return dl_handle_ptr(handle);
#else
static dl_handle_ptr dl_load_library(const std::string & path, bool silent) {
...
static dl_handle_ptr dl_load_library(const std::string & path, bool silent) {
    ...
    return dl_load_library(converter.from_bytes(path), silent);
}
#endif
...
struct ggml_backend_registry {
    ...
    ggml_backend_reg_t load_backend(const char * path, bool silent) {
        dl_handle_ptr handle = dl_load_library(path, silent);
        if (!handle) {
            return nullptr;
        }

        auto backend_init = (ggml_backend_init_t) dl_get_sym(handle.get(), "ggml_backend_init");
        if (!backend_init) {
            if (!silent) {
...
static ggml_backend_reg_t ggml_backend_load_best(const char * name, bool silent) {
    // enumerate all the files that match [lib]ggml-[name]-*.[so|dll]
        ...
        for (const auto & entry : fs::directory_iterator(search_path)) {
            if (entry.is_regular_file()) {
                ...
                if (filename.find(file_prefix) == 0 && ext == backend_filename_suffix()) {
                    dl_handle_ptr handle = dl_load_library(entry.path().c_str(), silent);
                    if (handle) {
